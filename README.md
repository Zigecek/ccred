...
```sh
curl -fsSLO https://github.com/Zigecek/ccred/releases/download/v0.3.11/ccred_0.3.11_amd64.deb
sudo apt install ./ccred_0.3.11_amd64.deb
```
...
```sh
curl -fsSLO https://github.com/Zigecek/ccred/releases/download/v0.3.11/ccred_0.3.11_amd64.deb.sha256
sha256sum -c ccred_0.3.11_amd64.deb.sha256
gh attestation verify ccred_0.3.11_amd64.deb --repo Zigecek/ccred
```
...
```sh
# Linux and macOS
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/Zigecek/ccred/releases/latest/download/ccred-installer.sh | sh
```

```powershell
# Windows
powershell -ExecutionPolicy Bypass -c "irm https://github.com/Zigecek/ccred/releases/latest/download/ccred-installer.ps1 | iex"
```
...
`%LOCALAPPDATA%\Packages\<family>\LocalCache\Roaming\Claude`, where the
family is `Claude_<hash>` for a sideloaded MSIX and
`AnthropicPBC.Claude_<hash>` for the Store's. A process outside the package
sees only the redirected path, so ccred looks in all of them -- both
families, the classic path, and the `Claude-3p` build's own names -- and
takes whichever holds a `config.json`, the most recently written one if
several do. `ccred doctor` names the directory it settled on. A Desktop profile is therefore a name, an
